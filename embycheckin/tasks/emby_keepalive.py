...
class EmbyKeepAliveTask(TaskHandler[EmbyKeepAliveConfig]):
    ...
    async def _authenticate(
        ...
        if cfg.api_key:
            try:
                ...
                logger.error(f"API key auth failed: HTTP {e.response.status_code}")
            except Exception as e:
    ...
    async def _simulate_playback(
        ...
        try:
            ...
            logger.info(f"[{ctx.task.name}] Started playback: {item_id}")
        ...
        for i in range(steps):
            ...
            position_ticks = (i + 1) * cfg.report_interval * 10_000_000
        ...
        try:
            ...
            logger.info(f"[{ctx.task.name}] Stopped playback: {item_id}")
